...
module.exports = async (req, res) => {
  ...
  await pushLead({
    ...
    tags: tagsFor("budget-tool").concat(flagged ? ["Needs Review"] : []),
    notes: [
      "Tool: Budget Builder",
      ...
      flagged ? "NEEDS REVIEW: " + (unverified ? "no reCAPTCHA token (posted outside the site form)" : "low reCAPTCHA score (" + lowScore + ")") + ". The spreadsheet was NOT generated or emailed. If this is a real person, follow up manually." : "",
    ],
  });
  ...
  if (flagged || spamRejected) {
    console.warn("[budget] low reCAPTCHA score", lowScore, "for", email, "- lead recorded + notified, generation skipped");
